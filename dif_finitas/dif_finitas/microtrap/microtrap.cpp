...
void InitChart (double rint[], double rext[], double z[], double velec[], int nelec, bool electrode[][MAXZ], 
                double V[][MAXZ], double dr, double dz, double rmax, double zmax,int contador, double paso);
...
int main (void)
{
	...
	for (contador=5;contador<=5;contador++){
		if(read){
			...
			InitChart(rint,rext,z,velec,nelec,electrode,V,dr,dz,rmax,zmax,contador,paso);
               ...
               double V[][MAXZ], double dr, double dz, double rmax, double zmax,int contador,double paso)
{
    ...
    for(int k=0;k<nelec;k++)
    {
    	...
    	for(int i=imin;i<=imax;i++)
    		...
    		{
    			electrode[i][j]=true;
    			if (k==0) V[i][j]=velec[k]+contador*paso;
                	else V[i][j]=velec[k];
    		}
    }
    return;
...
void Fit ()
{
	...
	fprintf(gp1,"fit f(x,y) 'microtrap-res.dat' using 1:2:3 via a,b,c,d\n");
	...
	fprintf(gp1,"set view map\n");
	fprintf(gp1,"set cntrparam levels 50\n");
	fprintf(gp1,"splot 'microtrap-res.dat' w pm3d linewidth 1, f(x,y)\n");
	fflush(gp1);
	pclose(gp1);
